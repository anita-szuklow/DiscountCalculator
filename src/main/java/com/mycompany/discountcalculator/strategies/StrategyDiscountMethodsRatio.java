...
public class StrategyDiscountMethodsRatio implements Strategy {
    @Override
    public String getName(){
        return "Sortowanie kandydatow po najlepszym stosunku upustu do ilosci dozwolonych rabatow";
    }
    @Override
    ...
    BigDecimal ratio;
        ...
        Candidate(Order order, PaymentMethod paymentMethod, BigDecimal ratio){
         this.order = order;
    ...
    for (Order order : orders){
        for(PaymentMethod paymentMethod : paymentMethods){
            ...
            BigDecimal discount = BigDecimal.valueOf(paymentMethod.discount);
            BigDecimal promotionsAllowed = BigDecimal.valueOf(order.promotions.size());
            BigDecimal ratio = discount.divide(promotionsAllowed, 6, RoundingMode.HALF_UP);
            candidates.add(new Candidate(order, paymentMethod, ratio));
        }
    }
